...
function Section({ id, kicker, title, children }: { id: string; kicker: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} className="mx-auto max-w-5xl px-6 py-14 border-t border-[var(--line)]">
      <p className="text-xs font-semibold uppercase tracking-widest text-[var(--accent)]">{kicker}</p>
      <h2 className="mt-2 text-2xl font-bold text-[var(--tx-1)]">{title}</h2>
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-[var(--line)] bg-[var(--inset)] p-5 transition-colors hover:border-indigo-500/40">
      {children}
    </div>
  );
}
...
export default function PortfolioPage() {
  return (
    <main className="min-h-screen bg-[var(--ground)] text-[var(--tx-2)] antialiased">
      {/* Hero */}
      <header className="relative overflow-hidden border-b border-[var(--line)]">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-20">
          <span className="inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-3 py-1 text-xs font-medium text-[var(--accent)]">
            ● 사내 실사용 · 단독 풀스택 개발
          </span>
          <h1 className="mt-6 text-4xl font-bold leading-tight text-[var(--tx-1)] sm:text-5xl">
            QA Dashboard
            <span className="block bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">
              AI 에이전트 기반 QA 자동화 플랫폼
            </span>
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[var(--tx-3)]">
            기획서 URL 한 줄로 <strong className="text-[var(--tx-1)]">TC 설계 → 작성 → 리뷰 → 수정</strong>까지
          ...
          <dl className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {STATS.map((s) => (
              <div key={s.label} className="rounded-xl border border-[var(--line)] bg-[var(--inset)] p-4">
                <dt className="text-2xl font-bold text-[var(--accent)]">{s.value}</dt>
                <dd className="mt-1 text-xs leading-snug text-[var(--tx-3)]">{s.label}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <Section id="problem" kicker="Problem" title="왜 만들었나">
        <p className="leading-relaxed text-[var(--tx-3)]">
          테스트 케이스 작성은 반복적이지만 작성자별 품질 편차가 크고, 리뷰는 사람 손에 의존해 커버리지 누락·중복을 놓치기 쉽다.
        </p>
        <blockquote className="mt-5 rounded-r-lg border-l-4 border-indigo-500 bg-indigo-500/5 px-5 py-4 text-[var(--tx-2)]">
      ...
      <Section id="pipeline" kicker="Focus ① — AI / Automation Design" title="다단계 에이전트 파이프라인">
        <p className="mb-6 leading-relaxed text-[var(--tx-3)]">
          Confluence URL 하나로 4개의 전문 에이전트가 릴레이로 동작한다. 각 단계는 직전 산출물을 입력받되
          <strong className="text-[var(--tx-1)]"> 세션은 독립</strong>시켜 컨텍스트 오염과 토큰 폭증을 막았다.
        </p>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {PIPELINE.map((p, i) => (
            <div key={p.mode} className="relative rounded-xl border border-[var(--line)] bg-[var(--inset)] p-5">
              <div className="absolute right-4 top-4 text-xs font-mono text-[var(--tx-4)]">0{i + 1}</div>
              <div className="text-2xl">{p.emoji}</div>
              <div className="mt-3 font-semibold text-[var(--tx-1)]">{p.label}</div>
              <div className="mt-0.5 font-mono text-[11px] text-[var(--accent)]/70">{p.mode}</div>
              <p className="mt-3 text-xs leading-relaxed text-[var(--tx-3)]">{p.out}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-3">
          <Card>
            <div className="text-sm font-semibold text-[var(--tx-1)]">프롬프트 = QA 방법론</div>
            <p className="mt-2 text-xs leading-relaxed text-[var(--tx-3)]">
              5개 모드별 시스템 프롬프트에 &quot;1TC=1검증&quot;, &quot;추상표현 금지&quot;, &quot;검증단계 49~60%&quot; 등 QA 규칙을 코드처럼 명문화.
            </p>
          </Card>
          <Card>
            <div className="text-sm font-semibold text-[var(--tx-1)]">MCP 오케스트레이션</div>
            <p className="mt-2 text-xs leading-relaxed text-[var(--tx-3)]">
              Jira·Confluence·Figma·GitHub·Slack 도구를 활용하되 원시 도구명을 한국어 라벨로 변환해 진행 상황 노출.
            </p>
          </Card>
          <Card>
            <div className="text-sm font-semibold text-[var(--tx-1)]">URL 원클릭 워크플로우</div>
            <p className="mt-2 text-xs leading-relaxed text-[var(--tx-3)]">
      ...
      <Section id="engineering" kicker="Focus ② — Engineering" title="까다로운 영역의 직접 해결">
        <p className="mb-6 leading-relaxed text-[var(--tx-3)]">
          자체 SDK 호출 코드가 아니라 <strong className="text-[var(--tx-1)]">CLI subprocess 스트림 파싱</strong>이라는 까다로운 영역에서
          실제 버그를 진단하고 해결했다.
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
          {ENGINEERING.map((e) => (
            <Card key={e.title}>
              <div className="text-sm font-semibold text-[var(--tx-1)]">{e.title}</div>
              <p className="mt-2 text-xs leading-relaxed text-[var(--tx-3)]">{e.body}</p>
              <code className="mt-3 inline-block rounded bg-[var(--line)] px-2 py-0.5 font-mono text-[11px] text-violet-300">
                {e.commit}
              </code>
            </Card>
          ))}
      ...
      <Section id="quality" kicker="Quality Engine" title="결정론적 TC 품질 채점">
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--line)]">
          <table className="w-full text-sm">
            <thead className="bg-[var(--panel-hi)] text-[var(--tx-3)]">
              <tr>
            ...
            <tbody>
              {QUALITY_CHECKS.map((c) => (
                <tr key={c.eval} className="border-t border-[var(--panel-hi)] hover:bg-[var(--panel-hi)]">
                  <td className="px-4 py-2.5 font-medium text-[var(--tx-1)]">{c.eval}</td>
                  <td className="px-4 py-2.5">
                    <span className="inline-flex h-6 min-w-[2rem] items-center justify-center rounded bg-indigo-500/15 px-2 font-mono text-xs text-[var(--accent)]">
                      {c.weight}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-[var(--tx-3)]">{c.how}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="mt-4 font-mono text-xs text-[var(--tx-3)]">
      ...
      <Section id="stack" kicker="Tech Stack" title="기술 스택">
        <div className="grid gap-3 sm:grid-cols-2">
          {STACK.map((s) => (
            <div key={s.area} className="rounded-xl border border-[var(--line)] bg-[var(--inset)] p-4">
              <div className="text-xs font-semibold uppercase tracking-wide text-[var(--accent)]">{s.area}</div>
              <div className="mt-1.5 text-sm text-[var(--tx-2)]">{s.items}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section id="impact" kicker="Impact" title="임팩트">
        <ul className="space-y-3">
          {[
            ...
            <li key={t} className="flex gap-3 text-sm leading-relaxed text-[var(--tx-2)]">
      ...
      <footer className="border-t border-[var(--line)] py-10 text-center text-xs text-[var(--tx-4)]">

...
|-------|--------|--------|---------|------------|---------|---------|
| 1 | 로그인 | 정상 로그인 | 유효한 계정 보유 | 1. 아이디 입력 2. 비밀번호 입력 3. 로그인 버튼 탭 | 메인 화면으로 이동 | 정상 |

- TC-ID는 숫자 순번(1, 2, 3…), 카테고리는 1depth/2depth 두 컬럼으로 분리
- 기대결과는 사용자가 보는 화면/동작 기준으로 기술 (함수명, 상태명 등 코드 레퍼런스 금지)
- Comment 컬럼에 검증단계(정상/부정/예외) 표기`,
...
## TC 출력 형식 (마크다운 테이블, 반드시 준수)

| TC-ID | 1depth | 2depth | 전제조건 | 테스트 스텝 | 기대결과 | Comment |
|-------|--------|--------|---------|------------|---------|---------|

- **1depth**: 대분류, **2depth**: 중분류 (소분류는 2depth에 포함하거나 테스트 스텝에 명시)
- **Comment**: 검증단계 표기 — 정상 / 부정 / 예외
...
| 추상 표현 금지 | "정상 동작하는지 확인" | "버튼 클릭 시 모달이 닫히는지 확인" |
| 재현스텝 3요소 필수 | "로그인하면 확인" | "[사전 상태]에서 [행동]하면 [구체적 결과]가 발생하는지 확인" |
| 경계값 수치 필수 | "마나가 부족할 때" | "마나 49/소모 50인 상태에서 스킬 사용 시 시전 차단 확인" |
| 플랫폼 미중복 | "PC에서 ~하면" (스텝에) | 플랫폼은 Comment에만 명시 |
...
- **EVAL-04**: 플랫폼 값이 PC / 모바일 / PC/모바일 외 없음
...
- **EVAL-09**: 1TC=1검증, 플랫폼 중복 없음, 부정+예외 49~60%, 경계값 수치 명시

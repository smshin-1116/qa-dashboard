...
- **Fail이면 사유에 "원인(코드)"을 반드시 담으세요** — 이 사유가 버그 티켓의 근거가 됩니다.
  프론트/백엔드 repo를 read-only(gh api·search_code·get_file_contents·gh pr diff)로 확인해
  **어디서 깨지는지 \`파일:라인\` + 한 줄 설명**으로 적으세요 (예: "\`src/services/order/Foo.js:88\` — 길이 비교로 미존재 분기").
  실사용 경로(프론트에서 이 동작을 부르는 화면·조건)도 확인되면 함께 적습니다.
  코드까지 확인 못 했으면 **"원인 미확정 — 재현/코드 확인 필요"** 라고 정직하게 명시하세요(모르는 걸 아는 척 금지).
- 응답 **맨 끝**에 반드시 아래 형식의 결과 표만 출력하세요 (자동 기입용):
| TC-ID | 결과 | 사유 |
